...
if (isset($_GET['id'])) {
    ...
    if ($_GET['id'] == 'all')
    {
        ...
        //  echo "1-22 Вы запросили ".$reqLimit." записей, начиная с конца";
            ...
            // LastID, limit,
            $stmt = $mysqli->prepare("SELECT * FROM $linksDBTable WHERE `id`<? ORDER BY `id` DESC LIMIT ?");

            // offset, limit
//            $stmt = $mysqli->prepare("SELECT * FROM $linksDBTable ORDER BY `id` DESC LIMIT ?,?");
...
//            $stmt->bind_param("ii",$reqOffset, $reqLimit);
    ...
    else
    {
        ...
        if (strlen($reqID) > 0) {
            ...
            {
                $stmt = $mysqli->prepare("SELECT * FROM $linksDBTable WHERE id=?");
                // info заменить на *
            ...
            elseif (count($goodReqIDArr) > 1)
            {
                ...
                $stmt = $mysqli->prepare("SELECT * FROM $linksDBTable WHERE id IN (" . $clause . ") GROUP BY `id`");
...
if (count($_POST) != 0) {
    if(isset($_POST['method'])) {
        ...
        if($_POST['method'] == "UPDATE") {
            ...
            if (isset($_POST['info']) && isset($_POST['id']) && $_POST['id'] !== "")
            {
                ...
                $newLinkTitle = trim($tendINFO->title);
                if (empty($newLinkTitle)) {
                    ...
                    $codedTitle = iconv("utf-8", "windows-1251", $newLinkTitle); // смена кодировки
                    $codedInfo = iconv("utf-8", "windows-1251", $_POST['info']);

                    $stmt = mysqli_prepare($link, "UPDATE $linksDBTable SET title=?, info=? WHERE id=?"); // создание строки запроса
        ...
        if($_POST['method'] == "DELETE") {
            ...
            if(isset($_POST['id']) && $_POST['id'] !== "") {
                ...
                $stmt = mysqli_prepare($link, "DELETE FROM $linksDBTable WHERE id=?");
    ...
    elseif (isset($_POST['info']))
    {
        ...
        else
        {
            ...
            $stmt = mysqli_prepare($link, "INSERT INTO $linksDBTable(title, info) VALUES (?, ?)"); // создание строки запроса
